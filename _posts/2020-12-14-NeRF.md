...
Several other approaches built on top of the implicit function idea, and generalize to training from 2D images. Of note are Structured Implicit Functions, CvxNet, Scene Representation Networks, Differentiable Volumetric Rendering, Deep Local Shapes, and the Implicit Differentiable Renderer.
...
One of the most interesting quotes from this paper hypothesizes about the success of neural volume rendering approaches (emphasis below is mine):
...
[AutoInt](http://www.computationalimaging.org/publications/automatic-integration/) greatly speeds up rendering by learning the volume integral directly.
